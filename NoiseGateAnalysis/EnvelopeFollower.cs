...
namespace NoiseGate
{
	class EnvelopeFollower
	{
		...
		private const double holdFc = 200.0;
		...
		public EnvelopeFollower(double fs, double releaseMs)
		{
			...
			var emaAlpha = (2 * Math.PI * ts * emaFc) / (2 * Math.PI * ts * emaFc + 1);
			...
			holdAlpha = (2 * Math.PI * ts * holdFc) / (2 * Math.PI * ts * holdFc + 1);
		}
		...
		public void ProcessEnvelope(double val)
		{
			...
			var movementValue = movementLatch.Update(sma.DbDecayPerSample > 0);
			...
			if (lastTriggerCounter > triggerCounterTimeoutSamples)
				...
				decay = Utils.Db2Gain(sma.DbDecayPerSample * 1.2); // 1.2 is fudge factor to make the follower decay slightly faster than actual signal, so we gently bump into the peaks
